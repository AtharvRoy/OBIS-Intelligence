...
// Fix: Explicitly use React.Component and typed interfaces for props and state to avoid 'state/props does not exist' errors.
class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    console.error("OBIS CRITICAL UI ERROR:", error, errorInfo);
  }

  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-slate-900 flex items-center justify-center p-12 text-center">
          <div className="max-w-xl w-full bg-white rounded-[3rem] p-16 shadow-2xl">
            <div className="w-20 h-20 bg-rose-100 text-rose-600 rounded-3xl mx-auto mb-8 flex items-center justify-center shadow-xl">
              <AlertTriangle className="w-10 h-10" />
            </div>
            <h1 className="text-3xl font-black text-slate-900 mb-4">Rendering Engine Fault</h1>
            <p className="text-slate-500 font-medium mb-10 leading-relaxed text-sm">
              The intelligence engine encountered an unexpected data structure. This is often caused by incomplete mock records or a version mismatch.
            </p>
            <div className="p-4 bg-slate-50 rounded-2xl mb-8 text-left border border-slate-200">
              <code className="text-[10px] text-rose-600 font-bold break-all">{this.state.error?.message}</code>
            </div>
            <button 
...
const App: React.FC = () => {
  ...
  const portfolioStats = useMemo(() => {
    return clients.map(client => {
      ...
      const isStale = (Date.now() - new Date(client.lastUpdatedAt || Date.now()).getTime()) > 1000 * 60 * 60 * 24 * 7;
  ...
  return (
    <ErrorBoundary>
      <div className="min-h-screen flex bg-slate-50" key={selectedClientId}>
